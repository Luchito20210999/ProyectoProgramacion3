...
public class ReclamosServiceRestClient : RestServiceClient<ReclamoItem, ReclamoRestDto>, IReclamosServiceClient
{
    ...
    protected override ReclamoItem ToViewModel(ReclamoRestDto source)
    {
        ...
        return new ReclamoItem
        {
            ...
            UsuarioResponsable = source.UsuarioResponsable ?? string.Empty,
            IdReserva = source.IdReserva,
            IdCliente = source.IdCliente,
            CodigoReserva = !string.IsNullOrWhiteSpace(source.CodigoReserva)
                ? source.CodigoReserva
                : source.IdReserva > 0 ? $"RES-{source.IdReserva}" : string.Empty,
            Cliente = !string.IsNullOrWhiteSpace(source.Cliente)
                ? source.Cliente
                : source.IdCliente > 0 ? $"Cliente #{source.IdCliente}" : "Cliente",
