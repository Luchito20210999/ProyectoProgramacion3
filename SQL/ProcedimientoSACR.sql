...
CREATE PROCEDURE sp_InsertCliente (
    ...
    IN p_tipo_documento VARCHAR(25),
...
BEGIN
    ...
    SELECT id_cliente INTO v_id_existente FROM Cliente 
    WHERE tipo_documento = p_tipo_documento AND numero_documento = p_numero_documento LIMIT 1;
    
    IF v_id_existente IS NOT NULL THEN
        UPDATE Cliente
        SET nombres = p_nombres, apellidos = p_apellidos, correo = p_correo,
            nacionalidad = p_nacionalidad, fecha_registro = p_fecha_registro,
            numero_contacto = p_numero_contacto, fecha_nacimiento = p_fecha_nacimiento, activo = p_activo
        WHERE id_cliente = v_id_existente;
        SET _id_generado = v_id_existente;
    ELSE
        INSERT INTO Cliente (nombres, apellidos, tipo_documento, numero_documento, correo, nacionalidad, fecha_registro, numero_contacto, fecha_nacimiento, activo)
        VALUES (p_nombres, p_apellidos, p_tipo_documento, p_numero_documento, p_correo, p_nacionalidad, p_fecha_registro, p_numero_contacto, p_fecha_nacimiento, p_activo);
...
BEGIN
    ...
    IF v_count > 0 THEN
        ...
        SET MESSAGE_TEXT = 'El documento ingresado ya pertenece a otro cliente.';
    ELSE
        UPDATE Cliente
        SET nombres = p_nombres, apellidos = p_apellidos, tipo_documento = p_tipo_documento,
            numero_documento = p_numero_documento, correo = p_correo, nacionalidad = p_nacionalidad,
            fecha_registro = p_fecha_registro, numero_contacto = p_numero_contacto,
            fecha_nacimiento = p_fecha_nacimiento, activo = p_activo
...
BEGIN
    ...
    SELECT id_usuario INTO v_id_existente FROM Usuario WHERE correo = p_correo LIMIT 1;
    
    IF v_id_existente IS NOT NULL THEN
        UPDATE Usuario 
        SET nombres = p_nombres, apellidos = p_apellidos, tipo_documento = p_tipo_documento, 
            numero_documento = p_numero_documento, contrasena = p_contrasena, 
            numero_contacto = p_numero_contacto, tipo_usuario = p_tipo_usuario, activo = p_activo
        WHERE id_usuario = v_id_existente;
        SET _id_generado = v_id_existente;
    ELSE
        INSERT INTO Usuario (nombres, apellidos, tipo_documento, numero_documento, correo, contrasena, numero_contacto, tipo_usuario, activo)
        VALUES (p_nombres, p_apellidos, p_tipo_documento, p_numero_documento, p_correo, p_contrasena, p_numero_contacto, p_tipo_usuario, p_activo);
        SET _id_generado = LAST_INSERT_ID();
    END IF;
END //
DELIMITER ;
...
BEGIN
    ...
    IF v_count > 0 THEN
        SIGNAL SQLSTATE '45000' 
        SET MESSAGE_TEXT = 'El correo ingresado ya pertenece a otro usuario en el sistema.';
    ELSE
        UPDATE Usuario
        SET nombres = p_nombres, apellidos = p_apellidos, tipo_documento = p_tipo_documento,
            numero_documento = p_numero_documento, correo = p_correo, contrasena = p_contrasena,
            numero_contacto = p_numero_contacto, tipo_usuario = p_tipo_usuario, activo = p_activo
...
BEGIN
    INSERT INTO Servicio (nombre, descripcion, precio_usd, duracion_horas, idioma_guia, capacidad_maxima, incluye_recojo, ciudad_destino, activo)
    VALUES (p_nombre, p_descripcion, p_precio_usd, p_duracion_horas, p_idioma_guia, p_capacidad_maxima, p_incluye_recojo, p_ciudad_destino, p_activo);
    SET _id_generado = LAST_INSERT_ID();
END //
DELIMITER ;
...
BEGIN
    UPDATE Servicio
    SET nombre = p_nombre, descripcion = p_descripcion, precio_usd = p_precio_usd,
        duracion_horas = p_duracion_horas, idioma_guia = p_idioma_guia, capacidad_maxima = p_capacidad_maxima,
        incluye_recojo = p_incluye_recojo, ciudad_destino = p_ciudad_destino, activo = p_activo
...
BEGIN
    UPDATE Notificacion
    SET mensaje = p_mensaje, tipo_notificacion = p_tipo_notificacion,
        fecha_envio = p_fecha_envio, leido = p_leido, id_usuario = p_id_usuario
...
BEGIN
    UPDATE Reserva
    SET fecha_registro = p_fecha_registro, estado_reserva = p_estado_reserva,
        cantidad_boletos = p_cantidad_boletos, monto_total = p_monto_total,
        fecha_ultima_modif = p_fecha_ultima_modif, canal_venta = p_canal_venta,
        monto_impuestos = p_monto_impuestos, codigo_bokun = p_codigo_bokun,
        id_usuario = p_id_usuario, id_cliente = p_id_cliente
...
DROP PROCEDURE IF EXISTS spListReservasDetalle;
DELIMITER //
CREATE PROCEDURE spListReservasDetalle()
BEGIN
    SELECT
        r.id_reserva,
        r.fecha_registro,
        ...
        COALESCE(det.id_servicio, 0) AS id_servicio,
        COALESCE(det.servicio, 'Reserva') AS servicio,
        det.ciudad_destino,
        COALESCE(det.precio_usd, 0) AS servicio_precio_usd,
        COALESCE(det.duracion_horas, 0) AS servicio_duracion_horas,
        COALESCE(det.capacidad_maxima, 0) AS servicio_capacidad_maxima,
        COALESCE(det.idioma_guia, '') AS servicio_idioma_guia,
        (COALESCE(det.incluye_recojo, 'N') = 'Y') AS servicio_incluye_recojo
    FROM Reserva r
    INNER JOIN Cliente c ON c.id_cliente = r.id_cliente
    LEFT JOIN (
        SELECT
            dr.id_reserva,
            MIN(dr.id_servicio) AS id_servicio,
            GROUP_CONCAT(s.nombre ORDER BY s.nombre SEPARATOR ', ') AS servicio,
            MIN(s.ciudad_destino) AS ciudad_destino,
            MIN(s.precio_usd) AS precio_usd,
            MIN(s.duracion_horas) AS duracion_horas,
            MIN(s.capacidad_maxima) AS capacidad_maxima,
            GROUP_CONCAT(DISTINCT s.idioma_guia ORDER BY s.idioma_guia SEPARATOR ', ') AS idioma_guia,
            MAX(s.incluye_recojo) AS incluye_recojo
        FROM Detalle_Reserva dr
        INNER JOIN Servicio s ON s.id_servicio = dr.id_servicio
        GROUP BY dr.id_reserva
    ) det ON det.id_reserva = r.id_reserva
    ORDER BY r.fecha_registro DESC, r.id_reserva DESC;
END //
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_ListReservaDetalleById;
DELIMITER //
CREATE PROCEDURE sp_ListReservaDetalleById (
    IN p_id_reserva INT
)
BEGIN
    SELECT
...
BEGIN
    ...
    SET id_reserva = p_id_reserva, id_servicio = p_id_servicio,
...
BEGIN
    UPDATE WebhookLog
    SET bookingId = p_bookingId, timestamp = p_timestamp,
        raw_data = p_raw_data, id_reserva = p_id_reserva
...
BEGIN
    UPDATE Reclamo
    SET fecha_reclamo = p_fecha_reclamo, descripcion = p_descripcion,
        estado_reclamo = p_estado_reclamo, motivo_resolucion = p_motivo_resolucion,
        fecha_resolucion = p_fecha_resolucion, id_usuario = p_id_usuario, id_reserva = p_id_reserva
...
DROP PROCEDURE IF EXISTS spListReclamosDetalle;
DELIMITER //
CREATE PROCEDURE spListReclamosDetalle()
BEGIN
    SELECT
        rec.id_reclamo,
        rec.fecha_reclamo,
        rec.descripcion,
        rec.estado_reclamo,
        rec.motivo_resolucion,
        rec.fecha_resolucion,
        rec.id_usuario,
        COALESCE(CONCAT(u.nombres, ' ', u.apellidos), 'Sistema') AS usuario_responsable,
        rec.id_reserva,
        COALESCE(NULLIF(r.codigo_bokun, ''), CONCAT('RES-', r.id_reserva)) AS codigo_reserva,
        r.id_cliente,
        CONCAT(c.nombres, ' ', c.apellidos) AS cliente
    ...
    INNER JOIN Cliente c ON c.id_cliente = r.id_cliente
    LEFT JOIN Usuario u ON u.id_usuario = rec.id_usuario
    ORDER BY rec.fecha_reclamo DESC, rec.id_reclamo DESC;
END //
DELIMITER ;

DROP PROCEDURE IF EXISTS sp_ListReclamoDetalleById;
DELIMITER //
CREATE PROCEDURE sp_ListReclamoDetalleById (
    IN p_id_reclamo INT
)
BEGIN
    SELECT
    ...
    INNER JOIN Cliente c ON c.id_cliente = r.id_cliente
    LEFT JOIN Usuario u ON u.id_usuario = rec.id_usuario
    WHERE rec.id_reclamo = p_id_reclamo;
END //
DELIMITER ;
...
BEGIN
    ...
    IF v_usr_correo IS NOT NULL AND v_usr_correo <> '' THEN
        ...
        FROM Usuario
        WHERE correo = v_usr_correo AND activo = TRUE
        LIMIT 1;
    ...
    WHILE i < v_array_len DO
        ...
        FROM Servicio
        WHERE nombre = v_srv_nombre AND activo = TRUE
        LIMIT 1;
