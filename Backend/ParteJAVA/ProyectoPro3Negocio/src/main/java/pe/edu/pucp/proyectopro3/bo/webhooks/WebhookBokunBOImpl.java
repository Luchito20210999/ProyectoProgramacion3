...
public class WebhookBokunBOImpl extends BaseBO implements WebhookBokunBO {
    ...
    @Override
    public void sincronizarBooking(String bookingId) {
        if (bookingId == null || bookingId.trim().isEmpty()) {
            throw new IllegalArgumentException(
                    "El bookingId de Bókun no puede estar vacío");
        }

        String bookingIdLimpio = bookingId.trim();

        try {
            BokunApiClient apiClient = BokunApiClient.desdeProperties();
            String jsonCompleto = apiClient.obtenerBookingCompleto(bookingIdLimpio);
            System.out.println("[WebhookBO] JSON completo obtenido desde Bókun ("
                    + jsonCompleto.length() + " chars)");

            this.webhookDAO.procesarWebhook(jsonCompleto);
            System.out.println("[WebhookBO] Booking " + bookingIdLimpio
                    + " sincronizado manualmente y persistido correctamente");

        } catch (Exception e) {
            throw new RuntimeException(
                    "Error al sincronizar booking de Bókun: "
                    + e.getMessage(), e);
        }
    }
