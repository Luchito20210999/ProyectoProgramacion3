...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        try (java.sql.Connection conn = pe.edu.pucp.proyectopro3.db.DBFactoryProvider.getManager().getConnection()) {
            java.sql.ResultSet rs = conn.getMetaData().getColumns(null, null, "Usuario", null);
            while (rs.next()) {
                System.out.println("COLUMNA: " + rs.getString("COLUMN_NAME") + " | TIPO: " + rs.getString("TYPE_NAME"));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
