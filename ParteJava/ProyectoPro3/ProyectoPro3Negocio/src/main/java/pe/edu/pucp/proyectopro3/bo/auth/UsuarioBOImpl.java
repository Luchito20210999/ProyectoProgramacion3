...
public class UsuarioBOImpl extends BaseBO implements UsuarioBO {
    ...
    public void crearUsuario(Usuario u) {
        ...
        Integer idGenerado = this.usuarioDao.crear(u);
        if (idGenerado == null || idGenerado <= 0) {
            throw new IllegalStateException("Error al registrar el usuario en el sistema.");
        }
    ...
    public void editarUsuario(int id) {
        ...
        Usuario usuario = this.usuarioDao.leer(id);
        ...
        if (!this.usuarioDao.actualizar(usuario)) {
            throw new IllegalStateException("No se pudo actualizar el usuario con ID: " + id);
        }
    }
    ...
    public void eliminar(int id) {
        eliminarUsuario(id); // Reutilizamos el método de negocio
    }
